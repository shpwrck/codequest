...
impl GameState {
    ...
    fn mastery_stage(&self, concept: Concept) -> usize {
        self.lens_record(concept)
            .stage_with(pending_reviews(self.lessons(), concept))
    }
...
/// full batch was survived, then forms new batches from the rest. Review
/// copies deferred for their retry gap are then placed in the grown deck.
...
fn present_current_question(state: &mut GameState) {
    ...
    let attempt = run
        ...
        .get(&identity)
        .copied()
        .unwrap_or(u32::from(question.review.is_review()));
...
mod tests {
    ...
    fn losing_the_last_heart_at_a_batch_boundary_does_not_level_up() {
        ...
        // the deck, the deferred copies are dropped, and the one outstanding
        // identity returns once as a same-launch review for the next run.
        ...
        assert_eq!(state.batch_ends, vec![1]);
    }
