...
/// regenerated stem is never queued beside its own review copy, and so are
/// stems the journal holds as answered correctly: a retired question stays
/// retired even when a delivery parked across runs brings it back.
fn append_question_batch(state: &mut GameState, questions: Vec<QuizQuestion>, level: u32) {
    ...
    let mut incoming = questions
        ...
        .map(|mut question| {
            // A stem the journal holds as missed is a redemption, whatever
            // the delivery's (possibly stale) flag says.
            question.review |= missed.contains(&question_identity(&question.question));
            question
        });
...
mod tests {
    ...
    fn a_parked_delivery_cannot_requeue_a_stem_the_journal_retired() {
        ...
        cartridge.questions.clear();
        record_lesson(&mut cartridge.lessons, &answered, true);
        ...
        assert_eq!(
            deck(&state),
            [
                (missed.question.clone(), true),
                (fresh.question.clone(), false)
            ],
        ...
        let mut state = GameState {
            cartridge: state.cartridge.take().map(|mut cartridge| {
                cartridge.questions.clear();
                record_lesson(&mut cartridge.lessons, &missed, true);
                cartridge
            }),
            ..Default::default()
        };
        let mut stale_review = missed.clone();
        stale_review.review = true;
    ...
    fn the_oracle_never_shows_the_answer_of_a_review_still_queued() {
        ...
        {
            let cartridge = state.cartridge.as_mut().unwrap();
            record_lesson(&mut cartridge.lessons, &cleared, true);
            record_lesson(&mut cartridge.lessons, &missed, false);
        ...
        assert!(deck[0].review && deck[0].question == missed.question);
        for ticks in 0..75 {
            state.screen_ticks = ticks;
