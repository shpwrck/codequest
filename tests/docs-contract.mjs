...
// Every retry waits the full gap (it carries into the next batch or waits
// for a delivery rather than coming back sooner).
const retryPhrase = new RegExp(`after (exactly )?${gap} other questions`);
for (const [path, text] of [
  ...
  assert.match(text, retryPhrase, `${path} must say a miss returns after ${gap} other questions`);
  assert.doesNotMatch(
    text,
    /up to \w+ intervening questions/,
    `${path} still says a retry can come back sooner than the gap`,
  );
}
